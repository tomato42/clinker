        ...
        if (clinker_url_protocol == "https:") {
            ...
            if (symetricCipher && symetricKeyLength ) {
                ...
                if (serverCert instanceof ci.nsIX509Cert) {
                    ...
                    var clinker_SubjectsPublicKey =
                        certificatesAll.getDisplayData(12).split(" ")[1]
                        .replace(/\(/g,'');
                    var clinker_CertificateSignatureAlgrithm =
                        certificatesAll.getDisplayData(certificatesAll.rowCount-2)
                        .replace(/PKCS #1/g,'').replace(/Encryption/g,'@');
                    var clinker_CertificateSignatureValue =
                        certificatesAll.getDisplayData(certificatesAll.rowCount-1)
                        .split(" ");

                    var clinker_SubjectsPublicKeyLocationCity = "",
                        clinker_SubjectsPublicKeyLocationState = "",
                        clinker_SubjectsPublicKeyLocationCountry="";
                    var clinker_SubjectsPublicKeyLocation =
                        certificatesAll.getDisplayData(9).split("\n");
                    for( i=0; i < clinker_SubjectsPublicKeyLocation.length; i++ ) {
                        if (clinker_SubjectsPublicKeyLocation[i].substring(0,3)
                            == "L =") {
                                clinker_SubjectsPublicKeyLocationCity =
                                    clinker_SubjectsPublicKeyLocation[i]
                                    .replace(/L =/g,'');
                        }
                        if (clinker_SubjectsPublicKeyLocation[i].substring(0,4)
                            == "ST =") {
                                clinker_SubjectsPublicKeyLocationState =
                                    clinker_SubjectsPublicKeyLocation[i]
                                    .replace(/ST =/g,'');
                    ...
                    var clinker_SubjectsCertificateLocationCity = "",
                        clinker_SubjectsCertificateLocationState = "",
                        clinker_SubjectsCertificateLocationCountry="";
                    var clinker_SubjectsCertificateLocation =
                        certificatesAll.getDisplayData(5).split("\n");
                    for( var i=0; i < clinker_SubjectsCertificateLocation.length ;
                        i++ ) {
                            if (clinker_SubjectsCertificateLocation[i]
                                .substring(0,3) == "L =") {
